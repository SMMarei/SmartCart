export const getAllShoppingLists = async () => {
  const response = await fetch(
    "http://localhost:4000/ShoppingLists/AllShoppingList",
  );
  if (!response.ok) {
...
export const createShoppingList = async (
  ...
  const response = await fetch(
    "http://localhost:4000/ShoppingLists/NewShoppingList",
    {
      method: "POST",
      headers: {
...
export const deleteShoppingList = async (id: string) => {
  ...
  if (!response.ok) {
    // return error message from the server/ backend
...
export const EditShoppingList = async (
  ...
  const response = await fetch(
    `http://localhost:4000/ShoppingLists/ShoppingList/${id}`,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
...
export const deleteItemFromShoppingList = async (
  ...
  const response = await fetch(
    `http://localhost:4000/ShoppingLists/ItemFromShoppingList/${listId}/${itemName}`,
    {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
